	...
	public void onPlayerItemCrafted(PlayerEvent.ItemCraftedEvent event)
	{
		...
		{               
			if(craftMatrix.getStackInSlot(i) != null) //If there is an item
			{
				...
				if(j.getItem() != null && (j.getItem() == RegistrationHandler.hammer || j.getItem() == RegistrationHandler.diamondHammer)){ //If it's a hammer
					ItemStack k;
					if(j.getItem() == RegistrationHandler.hammer)
						k = new ItemStack(RegistrationHandler.hammer, 2, (j.getItemDamage() + 1)); //Makes a new itemstack that's been damaged and has 2 items
					else
						k = new ItemStack(RegistrationHandler.diamondHammer, 2, (j.getItemDamage() + 1)); //Makes a new itemstack that's been damaged and has 2 items
